...
*/
CASE_OPCODE('N',"ReadName","Read Name",fputs(bam_get_qname(aln->b), ctx->out); if (aln->b->core.flag & BAM_FPAIRED) fputs((aln->b->core.flag & BAM_FREAD1 ?"/1":"/2"),ctx->out));
...
CASE_OPCODE('R',"REF","Ref base.",fputc(aln->ref_base,ctx->out));
