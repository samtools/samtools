Release 2.0.0 (June 2024)
--------------------------

**Major Release: CUDA GPU Acceleration & Codebase Refactor**

This release marks a major milestone for samtools, introducing GPU acceleration for key operations and a comprehensive codebase refactor for improved maintainability, performance, and extensibility.

### Highlights
- **CUDA GPU Acceleration**: Sorting, statistics, and pileup operations can now leverage NVIDIA GPUs for 3–5x speedup on large datasets. Automatic hardware detection and robust CPU fallback ensure seamless operation on all systems.
- **Modular CUDA Integration**: GPU code is cleanly separated in the `cuda/` directory, with device/context management, error handling, and memory optimization.
- **Auto-Detection & Fallback**: Build and runtime scripts detect GPU hardware and CUDA installation, providing clear user guidance and falling back to CPU if needed.
- **Improved Build System**: Enhanced `Makefile`, `configure.ac`, and `config.mk.in` for conditional CUDA support, auto-detection, and user-friendly configuration.
- **Comprehensive Documentation**: Unified and expanded `README.md` and `CUDA_IMPLEMENTATION.md` cover all usage, troubleshooting, and technical details.
- **Robust Testing**: New and improved scripts for GPU detection, build verification, and integration testing.
- **Centralized Error Handling**: New `sam_error.h` and `sam_error.c` for consistent, maintainable error management.
- **Codebase Refactor**: Improved modularity, readability, and maintainability; removed redundant/obsolete code and files; clarified header structure and comments.
- **Release Documentation**: Detailed `REFACTORING_SUMMARY.md` and updated release notes.

### Upgrade Notes
- No changes to file formats or command-line interface for existing features.
- GPU acceleration is optional and fully backward compatible.
- CPU-only mode remains the default if no compatible GPU is detected.

### Contributors
- Major CUDA integration and refactor by the AgenticAi team.
- Thanks to the samtools and HTSlib communities for foundational work and feedback.

---

For full details, see the unified `README.md` and `CUDA_IMPLEMENTATION.md`.
...
* Fix bug in split output format.  Now SAM and CRAM format can be chosen as well
